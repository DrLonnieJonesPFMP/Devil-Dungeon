...
(function(){
     ...
     Window_Base.prototype.addIconCaption = function(index,x,y) {
         ...
         if (this._iconCaptions.some(function(caption){return caption[0][0] === x && caption[0][1] === y && caption[2] === index})) {return}
         this._iconCaptions.push([[x,y],[x+Window_Base._iconWidth,y+Window_Base._iconHeight],index])
     }
     
     Window_Base.prototype.updateCaptions = function() {
         if (!$gameSwitches.value(Number(showCaptions))) {return}
         ...
         if (this._iconCaptions) {
             ...
             this._iconCaptions.forEach(function(caption){
                if (IconCaptions[caption[2]] && IconCaptions["cond"+caption[2]] !== undefined && !eval(IconCaptions["cond"+caption[2]])) {return}
                if (TouchInput._mouseX >= (caption[0][0] + x) && TouchInput._mouseX <= (caption[1][0] + x) && TouchInput._mouseY >= (caption[0][1] + y) && TouchInput._mouseY <= (caption[1][1] + y)) {
                    if (!SceneManager._scene._captionWindow) {
                        var captionWindow = new Window_IconCaption(caption[0][0] + x, caption[1][0] + y + win.lineHeight(),800,600)
                        ...
                        if (win instanceof Window_Gold && $gameParty.inBattle()) {return}
                        if (win instanceof Window_Gold && SceneManager._scene instanceof Scene_Map) {return}
                        ...
                        if (captionWindow.y + captionWindow.height > Graphics.boxHeight) {
                            captionWindow.y = caption[0][1] + y - win.lineHeight() * (1+ lineNum);
                        }
     ...
     Window_BattleStatus.prototype.updateCaptions = function() {
         if (SceneManager._scene._skillWindow.visible || SceneManager._scene._itemWindow.visible) {return}
    ...
    Window_IconCaption.prototype.update = function() {
        ...
        for (let i = 0; i < lines.length; i++) {
            if (maxWidth < this.textWidthEx(lines[i])) {maxWidth = this.textWidthEx(lines[i])}
        }
    ...
    Sprite_StateIcon.prototype.update = function() {
        ...
        if (!$gameSwitches.value(Number(showCaptions)) ||this._iconIndex === 0) {return}
        if (IconCaptions[this._iconIndex] && IconCaptions["cond"+this._iconIndex] !== undefined && !eval(IconCaptions["cond"+this._iconIndex])) {return}
        if (!this.visible) {return}
        var win = new Window_Base(800,600)
        var x = this.parent.x - Window_Base._iconWidth/2;
        var y = this.parent.y - Window_Base._iconHeight/2;
        if (TouchInput._mouseX >= (this.x + x) && TouchInput._mouseX <= (this.x + 48 + x) && TouchInput._mouseY >= (this.y + y) && TouchInput._mouseY <= (this.y + 48 + y)) {
                    if (!SceneManager._scene._captionWindow) {
                        var captionWindow = new Window_IconCaption(this.x + x, this.y + y + win.lineHeight(),800,600)
                        captionWindow.height = win.lineHeight() + win._padding * 2
                        captionWindow.width = 100;
                        captionWindow.hide();
                        SceneManager._scene._captionWindow = captionWindow
        ...
        if (Imported.MOG_BattleHud) {
            ...
            Battle_Hud.prototype.update_states2 = function() {
                ...
                this._stateIcons.forEach(function(icon) {
                    if (!$gameSwitches.value(Number(showCaptions)) ||icon._iconIndex === 0) {return}
                    if (IconCaptions[icon._iconIndex] && IconCaptions["cond"+icon._iconIndex] !== undefined && !eval(IconCaptions["cond"+icon._iconIndex])) {return}
                        if (!icon.visible) {return}
                        var win = new Window_Base(800,600)
                        var x = icon.parent.x
                        var y = icon.parent.y;
                        if (TouchInput._mouseX >= (icon.x + x) && TouchInput._mouseX <= (icon.x + 48 + x) && TouchInput._mouseY >= (icon.y + y) && TouchInput._mouseY <= (icon.y + 48 + y)) {
                                    if (!SceneManager._scene._captionWindow) {
                                        var captionWindow = new Window_IconCaption(icon.x + x, icon.y + y + win.lineHeight(),800,600)
                                        captionWindow.height = win.lineHeight() + win._padding * 2
                                        captionWindow.width = 100;
                                        captionWindow.hide();
                                        SceneManager._scene._captionWindow = captionWindow
                                        ...
                                        if (IconCaptions[icon._iconIndex]) {
                                            lineNum = IconCaptions[icon._iconIndex].split(/\r\n|\r|\n/).length
                                        }
                                        if (captionWindow.y + captionWindow.height > Graphics.boxHeight) {

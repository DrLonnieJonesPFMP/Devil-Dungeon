/*:
 *@author Synrec Codex
 *@plugindesc (v1.2) A state stacker plugin which indicates the amount
 ...
 *v1.1: Some minor fixes to the code.
 *v1.2: Removed the console logging.
 *
...
Game_BattlerBase.prototype.addStackableState = function(stateId){
	...
	var maxStack = eval($dataStates[stateId].meta.maxStack);
	var overflowState = eval($dataStates[stateId].meta.overflowState);
	if(isNaN(maxStack))maxStack = 1;
	...
	if (stack >= maxStack && !isNaN(overflowState)){
		this.addOverflowState(stateId, overflowState);
	}
}
